                ...
                for prod_setting in [1, 2]:
                    for prod_setting2 in [1, 2, 3]:
                        ...
                        for bud in range(1, total_budget + 1):
                            ...
                            top_order_list = ssto_main.generateTopOrderList(data_set_name, copy.deepcopy(wallet_list))
                            top_order_list = ssto_main.insertExpectProfitOrder(top_order_list, [set() for _ in range(num_product)])
                            top_order_list = ssto_main.insertOrder(top_order_list)
                            top_order_dict_o = ssto_main.constructTopOrderDict(top_order_list)
                            ...
                            for sample_count in range(sample_number):
                                ...
                                pro_acc = round(pro_acc / eva_monte_carlo, 2)
                                for kk in range(num_product):
                                    profit_k_list[kk] += round(pro_k_list_acc[kk] / eva_monte_carlo, 2)
                                ...
                                if (sample_count + 1) % sample_output_number == 0:
                                    path1 = "result/mtoic_pps" + str(pps) + "_wpiwp" * wpiwp
                                    if not os.path.isdir(path1):
                                        os.mkdir(path1)
                                    path = "result/mtoic_pps" + str(pps) + "_wpiwp" * wpiwp + "/" + data_set_name + "_" + product_name
                                    if not os.path.isdir(path):
                                        os.mkdir(path)
                                    fw = open(path + "/" + "b" + str(bud) + "_i" + str(sample_count + 1) + ".txt", 'w')
                                    fw.write("mtoic, pp_strategy = " + str(pps) + ", total_budget = " + str(bud) + ", wpiwp = " + str(wpiwp) + "\n" +
