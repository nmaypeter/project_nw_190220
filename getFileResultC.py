for data_setting in [1]:
    ...
    fw = open("result/r_" + data_set_name + "/comparison_test_profit.txt", 'w')
    for lnum, line in enumerate(profit):
        if lnum % (max_m * 3) == 0 and lnum != 0:
            fw.write("\n")
        if lnum % (max_m * 6) == 0 and lnum != 0:
            fw.write("\n")
        fw.write(str(line) + "\n")
    fw.close()
...
for data_setting in [1]:
    ...
    fw = open("result/r_" + data_set_name + "/comparison_profit.txt", 'w')
    for lnum, line in enumerate(profit):
        if lnum % (max_m * 3) == 0 and lnum != 0:
            fw.write("\n")
        if lnum % (max_m * 6) == 0 and lnum != 0:
