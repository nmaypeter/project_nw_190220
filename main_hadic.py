                ...
                for prod_setting in [1, 2]:
                    for prod_setting2 in [1, 2, 3]:
                        ...
                        for bud in range(1, total_budget + 1):
                            ...
                            sshad_main = SeedSelectionHAD(graph_dict, seed_cost_dict, product_list, bud)
                            ...
                            for sample_count in range(sample_number):
                                ...
                                degree_dict = sshad_main.constructAffordableDegreeDict(seed_set, copy.deepcopy(wallet_list))
                                mep_g, degree_dict = sshad_main.getHighDegreeNode(degree_dict, now_budget)
                                ...
                                if (sample_count + 1) % sample_output_number == 0:
                                    path1 = "result/mhadic_pps" + str(pps) + "_wpiwp" * wpiwp
                                    if not os.path.isdir(path1):
                                        os.mkdir(path1)
                                    path = "result/mhadic_pps" + str(pps) + "_wpiwp" * wpiwp + "/" + data_set_name + "_" + product_name
                                    if not os.path.isdir(path):
                                        os.mkdir(path)
                                    fw = open(path + "/" + "b" + str(bud) + "_i" + str(sample_count + 1) + ".txt", 'w')
                                    fw.write("mhadic, pp_strategy = " + str(pps) + ", total_budget = " + str(bud) + ", wpiwp = " + str(wpiwp) + "\n" +
